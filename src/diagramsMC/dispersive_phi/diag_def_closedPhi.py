...
class FuncNDiagNew:
    ...
    """
    def __init__(self,T,U,knum,taunum,nfreq,norder,perm,GFs,dependence):
        ...
        self.diag_coefficient=(-1)**nloopssig*(-self.U)**(self.norder)/self.knum**(3*(self.Ndimk))*(self.beta/self.taunum)**(self.Ndimtau)
    ...
    def update(self,new_momentum,new_imagtime,newsublatindshort):
        '''
    ...
    def update_temp(self,iloop,new_momentum,new_imagtime,newsublatindshort,globalflip=0):
        '''
       ...
       '''
        ...
        if iloop<self.Ndimk:# changing variable k 
            ...
            result=prod_phi(self.slicedG_temp)
            time3=time.time()
...
@jit(nopython=True, cache=True, fastmath=True)
def prod_phi(slices):# to get phi,seems we do not need any basis? since everything is integrated out.
    return np.prod(slices)